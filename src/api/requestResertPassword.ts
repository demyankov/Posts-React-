...
export interface RequestResertPasswordType {
  ...
  export async function requestResertPassword ({...querryBody}:RequestResertPasswordType):
  Promise<any> {
  ...
  if (!response.ok) {await Promise.reject({
     global: ['Request reset password failed!'] 
  })};
}
